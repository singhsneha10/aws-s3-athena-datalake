...
Purpose:
    ...
    and one fact table — all as Athena views (no physical storage cost).
...
Views always reflect the latest Silver data without a separate load step.
===============================================================================
*/
...
-- Filters out historical products (only current prd_end_dt IS NULL).
-- Assigns surrogate key via ROW_NUMBER().

DROP VIEW IF EXISTS gold.dim_products;
CREATE VIEW gold.dim_products AS
SELECT
    ROW_NUMBER() OVER (ORDER BY pr.prd_id)  AS product_key,    -- Surrogate key
...
-- ============================================================
-- Uncomment to verify Gold layer is populated correctly.
